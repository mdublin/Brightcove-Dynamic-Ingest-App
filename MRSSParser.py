...
# For Dynamic Ingest API, see here:
# http://docs.brightcove.com/en/video-cloud/di-api/samples/batch-dynamic-ingest.html
...
d = feedparser.parse({INSERT YOUR FEED HERE})
response_array = []
...
for index, post in enumerate(d.entries):
    ...
    # character check for description, BC short description field only allows
    # for 250 characters:
    ...
    # tag cleanup, sometimes your feed has empty tags like " ", Brightcove's
    # Dynamic Ingest API cannot handle empty tags and will totally reject the
    # asset
    tags = item['tags']
    ...
    # the element with the video URL in your feed, usually called "link" or
    # "url"
...
for item in response_array:
    ...
    if not BC_01.videoNameExists(name):  # Calling our dedupe function
        ...
        # If no duplicates, call our Brightcove Dynamic Ingest function
        create_video = BC_01.createAndIngest(name, url, tags, desc)
        #store in db
        bcid = create_video[0]
        ingest_job_response = create_video[1]
        ingest_job_response = ingest_job_response['id']

        vid_store = models.Video(
            video_name=name,
            Source_URL=url,
            DynamicIngest_response=ingest_job_response,
            BCid=bcid
        )
